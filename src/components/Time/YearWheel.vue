...
const angle = computed(() => {
  if (store.dayOfYear >= 360) return 0; // lock to zen
  return (store.dayOfYear / 360) * 360;
})

function goToHabit(habitIndex: number) {
  const now = DateTime.fromJSDate(store.now);
  const start = store.zenContext.zenYearStart;

  const dayIndex = habitIndex * 45;
...
function goToZen() {
  ...
  // go to first zen day (day 360)
  const target = start.plus({ days: 360 });
  ...
  <div class="zenwheel pointer" style="cursor: pointer" @click="emit('year')">
    ...
    <circle class="habit-divider"
      ...
      />
        ...
        <g v-for="(habit, i) in store.habitNames.slice(0, 8)" :key="i">
          <text
            :class="store.habitNum === i ? 'active' : ''"
            text-anchor="middle"
            dominant-baseline="middle"
            :x="200 + Math.cos((i * 45 + 22.5 - 90) * Math.PI/180) * 120"
            :y="300 + Math.sin((i * 45 + 22.5 - 90) * Math.PI/180) * 120"
            style="cursor: pointer;"
            @click.stop="goToHabit(i)"
        >{{ habit }}</text>
        </g>
      ...
      <g>
        ...
        <text x="200" y="300" class="habit-names"
          text-anchor="middle" dominant-baseline="middle">Day {{store.dayOfYear+1}}</text>
      </g>



      <g class="season-icon"
        :transform="`translate(${59} ${406}) scale(0.06)`"
      >
        <g id="sun">
      ...
      <g class="season-icon"
        :transform="`translate(${65} ${159}) scale(2)`"
      >
        <path
      ...
      <g class="season-icon"
        :transform="`translate(314 167) scale(1)`"
      >
      ...
      <g id="_x32_" class="season-icon"
        :transform="`translate(${309} ${414}) scale(0.05)`"
        >
        <g>
