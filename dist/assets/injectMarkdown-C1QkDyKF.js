import { ah as getDefaultExportFromCjs } from "./index-BBZgqMci.js";
const decodeCache = {};
...
function requireUtils() {
  ...
  const HTML_ESCAPE_TEST_RE2 = /[&<>"]/;
  const HTML_ESCAPE_REPLACE_RE2 = /[&<>"]/g;
  const HTML_REPLACEMENTS2 = {
    "&": "&amp;",
    "<": "&lt;",
    ">": "&gt;",
    '"': "&quot;"
...
function requirePatterns() {
  ...
  patterns = (options) => {
    const __hr = new RegExp("^ {0,3}[-*_]{3,} ?" + utils2.escapeRegExp(options.leftDelimiter) + "[^" + utils2.escapeRegExp(options.rightDelimiter) + "]");
    return [
      {
        /**
         * ```python {.cls}
        ...
        tests: [
          {
            shift: 0,
            block: true,
            info: utils2.hasDelimiters("end", options)
          }
        ],
        transform: (tokens, i) => {
          const token = tokens[i];
          const start = token.info.lastIndexOf(options.leftDelimiter);
          const attrs = utils2.getAttrs(token.info, start, options);
          utils2.addAttrs(attrs, token);
          token.info = utils2.removeDelimiter(token.info, options);
        }
      },
      {
        /**
        ...
        tests: [
          {
            ...
            children: [
              ...
              },
              {
                shift: 0,
                type: "text",
                content: utils2.hasDelimiters("start", options)
              }
            ]
          }
        ],
        ...
        transform: (tokens, i, j) => {
          ...
          const attrs = utils2.getAttrs(token.content, 0, options);
          utils2.addAttrs(attrs, attrToken);
      ...
      },
      {
        ...
        tests: [
          ...
          },
          {
            shift: 2,
            type: "inline",
            content: utils2.hasDelimiters("only", options)
          }
        ],
        transform: (tokens, i) => {
          const token = tokens[i + 2];
          const tableOpen = utils2.getMatchingOpeningToken(tokens, i);
          const attrs = utils2.getAttrs(token.content, 0, options);
          utils2.addAttrs(attrs, tableOpen);
          tokens.splice(i + 1, 3);
        }
      },
      {
        ...
        transform: (tokens, i) => {
          const tr = utils2.getMatchingOpeningToken(tokens, i);
          const th = tokens[i - 1];
          let colsnum = 0;
          let n = i;
          while (--n) {
      ...
      },
      {
        ...
        transform: (tokens, i, j) => {
          ...
          const attrs = utils2.getAttrs(content, 0, options);
          const openingToken = utils2.getMatchingOpeningToken(tokens[i].children, j - 1);
          utils2.addAttrs(attrs, openingToken);
      ...
      },
      {
        ...
        transform: (tokens, i, j) => {
          ...
          utils2.addAttrs(attrs, tokens[ii - 1]);
      ...
      },
      {
        ...
        transform: (tokens, i) => {
          ...
          const content = token.content;
          const attrs = utils2.getAttrs(content, 0, options);
          const openingToken = utils2.getMatchingOpeningToken(tokens, i);
          utils2.addAttrs(attrs, openingToken);
          tokens.splice(i + 1, 3);
        }
      },
      {
            ...
            children: [
              {
                position: -1,
                type: "text",
                content: utils2.hasDelimiters("end", options)
              }
            ]
          }
        ],
        ...
        transform: (tokens, i, j) => {
          const token = tokens[i].children[j];
          const content = token.content;
          const attrs = utils2.getAttrs(content, content.lastIndexOf(options.leftDelimiter), options);
          utils2.addAttrs(attrs, tokens[i - 2]);
      ...
      },
      {
        ...
        transform: (tokens, i, j) => {
          ...
          const openingToken = utils2.getMatchingOpeningToken(tokens, ii);
          utils2.addAttrs(attrs, openingToken);
          tokens[i].children = tokens[i].children.slice(0, -2);
        }
      },
      {
        ...
        transform: (tokens, i) => {
          ...
          const attrs = utils2.getAttrs(content, start, options);
          utils2.addAttrs(attrs, token);
...
var markdownItAttrs$1;
var hasRequiredMarkdownItAttrs;
function requireMarkdownItAttrs() {
  if (hasRequiredMarkdownItAttrs) return markdownItAttrs$1;
  ...
  markdownItAttrs$1 = function attributes(md, options_) {
    let options = Object.assign({}, defaultOptions2);
    options = Object.assign(options, options_);
    const patterns2 = patternsConfig(options);
    function curlyAttrs(state) {
      const tokens = state.tokens;
      for (let i = 0; i < tokens.length; i++) {
        for (let p = 0; p < patterns2.length; p++) {
          const pattern = patterns2[p];
          let j = null;
          ...
          if (match2) {
            try {
              ...
              console.error(`markdown-it-attrs: Error in pattern '${pattern.name}': ${error2.message}`);
  ...
  return markdownItAttrs$1;
}
var markdownItAttrsExports = requireMarkdownItAttrs();
const markdownItAttrs = /* @__PURE__ */ getDefaultExportFromCjs(markdownItAttrsExports);
const injectMarkdown = (rawMd, sectionName) => {
  const md = new MarkdownIt().use(markdownItAttrs);
  let markdownToRender = rawMd;
  if (sectionName) {
    const pattern = new RegExp(`## \\[${sectionName}\\]([\\s\\S]*?)(?=## \\[|$)`);
    const match2 = rawMd.match(pattern);
    markdownToRender = match2 ? match2[1].trim() : "";
  }
